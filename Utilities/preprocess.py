...
def saveImage(image: np.ndarray, fileName: str, outDir: str) -> int:
    if image.shape[0] == 0:
        ...
        if imgShape[1] == 64:
            ...
            if '64_32' not in os.listdir(outDir):
                os.mkdir(os.path.join(outDir, '64_32'))

            mpimage.imsave(os.path.join(outDir, '64_32', fileName), image)

        elif imgShape[1] == 128:

            if '128_64' not in os.listdir(outDir):
                os.mkdir(os.path.join(outDir, '128_64'))

            mpimage.imsave(os.path.join(outDir, '128_64', fileName), image)

        elif imgShape[1] == 208:
            
            if '192_96' not in os.listdir(outDir):
                os.mkdir(os.path.join(outDir, '192_96'))
